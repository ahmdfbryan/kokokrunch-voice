...
const commands = [
  {
    ...
    async execute(interaction, log) {
      ...
      if (startedImmediately) {
        await interaction.editReply(`Started playing **${track.title}**`);
      } else {
        await interaction.editReply(`**${track.title}** ditambahkan ke antrian (posisi #${position})`);
      }
    },
  },

  {
    ...
    async execute(interaction) {
      ...
      if (!skipped) {
        await interaction.reply({ content: 'Nggak ada lagu yang lagi diputar.', ephemeral: true });
        return;
      }
      await interaction.reply(`**${skippedTrack.title}** has been skipped by <@${interaction.user.id}>`);
    },
  },

  {
    ...
    async execute(interaction) {
      ...
      if (!hadSomething) {
        await interaction.reply({ content: 'Nggak ada musik yang lagi diputar atau diantrikan.', ephemeral: true });
        return;
      }
      await interaction.reply('Musik dihentikan, antrian dikosongkan.');
    },
  },
